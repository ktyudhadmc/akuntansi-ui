...
export default function useGetReportCustomerBalance() {
  ...
  const { start_date, end_date } = parseMonthAndRange(saleCustomerBalanceDate);
  ...
  const { data, error } = useSWR(`/reports/customer-balance?${qs}`, fetcher);
  ...
  const DUMMY_DATA: IGetReportCustomerBalance = {
    ...
    pagination: {
      current_page: 1,
      first_page_url: "https://api.example.com/reports/customer-balance?page=1",
      from: 1,
      last_page: 1,
      last_page_url: "https://api.example.com/reports/customer-balance?page=1",
      links: [
        { url: null, label: "&laquo; Previous", page: null, active: false },
        {
          url: "https://api.example.com/reports/customer-balance?page=1",
          label: "1",
          page: 1,
          active: true,
        },
      ...
      next_page_url: null,
      path: "https://api.example.com/reports/customer-balance",
      per_page: 10,
      prev_page_url: null,
      to: 2,
      total: 2,
