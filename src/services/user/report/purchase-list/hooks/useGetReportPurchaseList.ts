...
import type { IGetReportPurchaseListResponse } from "../interfaces/response.type";

export default function useGetReportPurchaseList() {
  const [search, setSearch] = useState("");
  const [pageNum, setPageNum] = useState(1);
  const [pageLimit, setPageLimit] = useState(10);
  const purchaseListDate = useUserStore((state) => state.purchaseListDate);

  const { start_date, end_date } = parseMonthAndRange(purchaseListDate);

  const fetcher: Fetcher<IGetReportPurchaseListResponse, string> = (url) =>
  ...
  const { data, error } = useSWR(`/reports/purchase-list?${qs}`, fetcher);

  const onSetSearch = useCallback((search: string) => {
    setSearch(search);
  }, []);

  const DUMMY_DATA: IGetReportPurchaseListResponse = {
    summary: {
    ...
    data: [
      {
        ...
        supplier: { id: 1, name: "Budi Santoso" },
        note: "Catatan 1",
      ...
      },
      {
        ...
        supplier: { id: 2, name: "Siti Rahayu" },
        note: "Catatan 2",
      ...
      },
      {
        ...
        supplier: { id: 3, name: "Ahmad Wijaya" },
        note: "",
