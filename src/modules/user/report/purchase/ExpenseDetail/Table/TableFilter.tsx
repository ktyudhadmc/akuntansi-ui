...
export default function TableFilter() {
  ...
  const setPurchaseExpenseDetailDate = useUserStore(
    (state) => state.setPurchaseExpenseDetailDate,
  );

  const urlExports = [
    {
      label: "pdf",
      onClick: () => toast.info("PDF"),
    },
    {
      label: "csv",
      onClick: () => toast.info("CSV"),
    },
    {
      label: "xlsx",
      onClick: () => toast.info("XLSX"),
    },
  ];
  ...
  const onSubmit: SubmitHandler<any> = async (state) => {
    setPurchaseExpenseDetailDate(state.date);
  };
  ...
  //     date: todayYMString,
  //   });
  //   setPurchaseExpenseDetailDate(todayYMString);
  // };
        ...
        <Form {...methods} onSubmit={onSubmit}>
          <div className="grid lg:grid-cols-4 md:grid-cols-2 gap-2 items-end">
            <Input
              name="date"
              type="month"
              defaultValue={purchaseExpenseDetailDate}
            />
